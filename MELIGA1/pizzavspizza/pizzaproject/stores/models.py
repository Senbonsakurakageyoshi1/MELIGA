...
class TODO(models.Model):
    ...
    phone_number=models.CharField(validators=[RegexValidator(regex=r'^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$')],max_length=10,blank=True)
    description=models.TextField(blank=True)
    logo_image=models.ImageField(upload_to='pizzariaImages',blank=True,default="pizzariaImages/pizzalogo.png")
    emial=models.EmailField(max_length=245,blank=True)
